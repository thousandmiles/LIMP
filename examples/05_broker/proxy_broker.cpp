/**
 * @file proxy_broker.cpp
 * @brief Example demonstrating ZeroMQ Proxy for automatic message brokering
 *
 * This example shows how to use the ZMQProxy class to create a central
 * message broker that automatically forwards messages between multiple nodes.
 * Uses ROUTER-ROUTER pattern for N:N bidirectional communication.
 */
...
int main()
{
    ...
    // Configure frontend endpoint (nodes connect here)
    // For ROUTER-ROUTER broker, frontend and backend must be different endpoints
    ...
    // Configure backend endpoint (other nodes connect here)
    // Using separate endpoint for backend allows bidirectional routing
    std::string backendEndpoint = "tcp://*:5556";
    ...
    std::cout << "  Note: Nodes can connect to either frontend or backend" << std::endl;
    std::cout << std::endl;
